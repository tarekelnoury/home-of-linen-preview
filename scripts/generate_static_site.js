...
function nav(locale, pathname) {
  ...
  return `${items}<a href="${hrefTo(locale, pathname, locale, "/about/")}">${locale === "ar" ? "عن القطن المصري" : "About"}</a><a href="${hrefTo(locale, pathname, locale, "/bridal-trousseau/")}">${locale === "ar" ? "جهاز العروسة" : "Bridal Trousseau"}</a><a href="${hrefTo(locale, pathname, locale, "/guides/")}">${locale === "ar" ? "الدليل" : "Guides"}</a>`;
}
...
function productCardStatic(product, locale, pathname) {
  ...
  return `<article class="card" data-product="${esc(product.id)}">
    ...
    <div class="card-body"><div><p class="eyebrow">${esc(categoryName(product.category, locale))}</p><h3><a href="${productHref}">${esc(productName(product, locale))}</a></h3></div><p class="spec" data-spec>${esc(product.includes || "")}</p><div data-controls></div><div class="buy-row"><div><div class="price" data-price>${firstPrice(product) ? `EGP ${firstPrice(product).toLocaleString("en-EG")}` : ""}</div><div class="qty" aria-label="Quantity selector"><button type="button" data-qty-down>−</button><span data-qty>1</span><button type="button" data-qty-up>+</button></div></div><button class="add-button" type="button" data-add>${product.noPrice ? (locale === "ar" ? "قريباً" : "Coming Soon") : (locale === "ar" ? "أضف للسلة" : "Add to Basket")}</button></div><p class="description" data-description>${esc(productDescription(product, locale))}</p></div>
  </article>`;
}
