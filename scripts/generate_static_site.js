...
function nav(locale) {
  ...
  const items = Object.entries(categorySlugs).map(([label, slug]) =>
    `<a href="${base}/categories/${slug}/">${esc(categoryName(label, locale))}</a>`
  ).join("");
  return `${items}<a href="${base}/about/">${locale === "ar" ? "عن القطن المصري" : "About"}</a><a href="${base}/bridal-trousseau/">${locale === "ar" ? "جهاز العروسة" : "Bridal Trousseau"}</a><a href="${base}/guides/">${locale === "ar" ? "الدليل" : "Guides"}</a>`;
}
...
function layout({ locale = "en", type = "content", title, description, pathname, h1, body, catalogHtml = "", config = {}, schema = [], ogImage = "/assets/images/og-default.jpg" }) {
  ...
  const pageConfig = { locale, type, assetBase: "/", ...config };
  return `<!doctype html>
<html lang="${locale}" dir="${dir}">
  <head>
    ...
    <link rel="icon" href="/assets/brand/home-of-linen-logo.png" />
  ...
  <body>
    <header class="site-header">
      <a class="brand" href="${locale === "ar" ? "/ar/" : "/"}" aria-label="Home of Linen home">
        <img src="/assets/brand/home-of-linen-logo.png" alt="Home of Linen logo" />
      </a>
      <nav class="nav" aria-label="${locale === "ar" ? "أقسام المنتجات" : "Product categories"}">${nav(locale)}</nav>
...
function productCardStatic(product, locale) {
  const image = productImage(product);
  return `<article class="card" data-product="${esc(product.id)}">
    <a class="image-wrap" data-image href="${locale === "ar" ? "/ar" : ""}/products/${productSlug(product)}/"><img src="/${esc(image)}" alt="${esc(productName(product, locale))} ${locale === "ar" ? "من هوم أوف لينن" : "by Home of Linen"}" loading="lazy"></a>
    <div class="mobile-controls" data-mobile-controls></div>
    <div class="card-body"><div><p class="eyebrow">${esc(categoryName(product.category, locale))}</p><h3><a href="${locale === "ar" ? "/ar" : ""}/products/${productSlug(product)}/">${esc(productName(product, locale))}</a></h3></div><p class="spec" data-spec>${esc(product.includes || "")}</p><div data-controls></div><div class="buy-row"><div><div class="price" data-price>${firstPrice(product) ? `EGP ${firstPrice(product).toLocaleString("en-EG")}` : ""}</div><div class="qty" aria-label="Quantity selector"><button type="button" data-qty-down>−</button><span data-qty>1</span><button type="button" data-qty-up>+</button></div></div><button class="add-button" type="button" data-add>${product.noPrice ? (locale === "ar" ? "قريباً" : "Coming Soon") : (locale === "ar" ? "أضف للسلة" : "Add to Basket")}</button></div><p class="description" data-description>${esc(productDescription(product, locale))}</p></div>
  </article>`;
}
...
function productPage(product, locale) {
  ...
  const catalogHtml = `<div class="grid">${productCardStatic(product, locale)}</div>`;
  const productSchema = {
    "@context": "https://schema.org",
    "@type": "Product",
    name,
...
function guideRelatedLinks(pathname, locale) {
  ...
  return `<nav class="guide-links" aria-label="${esc(label)}"><p class="eyebrow">${esc(label)}</p>${categories.map(category => `<a href="${prefix}/categories/${categorySlugs[category]}/">${esc(categoryName(category, locale))}</a>`).join("")}</nav>`;
}
...
function homePage(locale) {
  ...
  const body = `<h1 class="visually-hidden">${esc(h1)}</h1>
      <section class="hero" aria-label="${esc(h1)}">
        <img class="hero-image" src="/assets/images/main-hero-image.png" alt="${esc(locale === "ar" ? "سرير بمفروشات قطن مصري بيضاء من Home of Linen في غرفة هادئة" : "white Egyptian cotton bedding in a warm Home of Linen bedroom")}" />
        <a class="hero-hotspot" href="${locale === "ar" ? "/ar/categories/fitted-sheets/" : "/categories/fitted-sheets/"}" aria-label="${esc(locale === "ar" ? "تسوق مفروشات القطن المصري" : "Shop Egyptian cotton bedding")}"></a>
      </section>`;
  return {
    locale,
    pathname: "/",
...
function generate() {
  ...
  const bridalText = {
    en: ["A bridal trousseau should feel complete, useful and beautiful. Home of Linen makes it easier to build a calm bedroom bundle with fitted sheets, flat sheets, duvet covers, pillowcases and towels in coordinated colours.", "Start with the mattress size, add matching pillowcases and duvet covers, then complete the bathroom with plush towels. Send your selected basket on WhatsApp and we will help confirm the details."],
    ar: ["جهاز العروسة يجب أن يكون كاملاً وعملياً وجميلاً. تساعدك Home of Linen على بناء مجموعة هادئة لغرفة النوم تشمل ملايات أستك، ملايات سادة، أغطية لحاف، أكياس مخدات وفوط بألوان متناسقة.", "ابدئي بمقاس المرتبة، ثم أضيفي أكياس المخدات وأغطية اللحاف المطابقة، وأكملي الحمام بفوط ناعمة. أرسلي السلة على واتساب وسنساعدك في تأكيد التفاصيل."]
  };
  pages.push(contentPage("en", "/bridal-trousseau/", "Bridal Trousseau Bedding in Egypt | Home of Linen", "جهاز العروسة مفروشات قطن مصري | Home of Linen", "Build a bridal trousseau with Egyptian cotton sheets, duvet covers and towels. Choose calm colours and order on WhatsApp today.", "جهزي جهاز العروسة بملايات قطن مصري وأغطية لحاف وفوط بألوان هادئة. اختاري واطلبي عبر واتساب اليوم.", "Bridal Trousseau Bedding in Egypt", "جهاز العروسة من مفروشات القطن المصري", bridalText));
  pages.push(contentPage("ar", "/bridal-trousseau/", "Bridal Trousseau Bedding in Egypt | Home of Linen", "جهاز العروسة مفروشات قطن مصري | Home of Linen", "Build a bridal trousseau with Egyptian cotton sheets, duvet covers and towels. Choose calm colours and order on WhatsApp today.", "جهزي جهاز العروسة بملايات قطن مصري وأغطية لحاف وفوط بألوان هادئة. اختاري واطلبي عبر واتساب اليوم.", "Bridal Trousseau Bedding in Egypt", "جهاز العروسة من مفروشات القطن المصري", bridalText));
